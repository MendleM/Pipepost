...
describe("publishToDevto", () => {
  ...
  it("includes canonical_url when provided", async () => {
    mockHttp.httpRequest.mockResolvedValue({
      success: true,
      data: { id: 1, url: "https://dev.to/user/post" },
    });
    ...
    await publishToDevto(
      { title: "Test", content: "Body", canonical_url: "https://myblog.com/post" },
      "my-key"
    );
    ...
    expect(body.article.canonical_url).toBe("https://myblog.com/post");
  });

  it("omits canonical_url when not provided", async () => {
    mockHttp.httpRequest.mockResolvedValue({
      success: true,
      data: { id: 1, url: "https://dev.to/user/post" },
    });
    ...
    expect(body.article.canonical_url).toBeUndefined();
  });
  ...
  it("defaults tags to empty array when not provided", async () => {
    ...
    await publishToDevto({ title: "Test", content: "Body" }, "my-key");
...
describe("listDevtoPosts", () => {
  ...
  it("returns error on API failure", async () => {
    ...
    const result = await listDevtoPosts("bad-key");
  ...
  it("passes pagination params", async () => {
    ...
    await listDevtoPosts("my-key", 2, 10);

    const url = mockHttp.httpRequest.mock.calls[0][0];
