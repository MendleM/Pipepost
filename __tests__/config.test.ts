...
describe("writeConfig", () => {
  it("creates directory and writes JSON", () => {
    ...
    // Find the config write (not the lock file write which uses { flag: "wx" })
    const configWriteCall = mockFs.writeFileSync.mock.calls.find(
      (call) => (call[0] as string).endsWith("config.json")
    );
    expect(configWriteCall).toBeDefined();
    const writtenJson = JSON.parse(configWriteCall![1] as string);
